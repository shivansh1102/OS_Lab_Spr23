...
Pipe::Pipe(string &command) : fullCmd(trim(command)), individualCmds(getAllPipes(fullCmd)), pgrpID(0) 
{
    if(fullCmd.back() == '&')
    isBackground = true;
...
void Pipe::executePipe()
{
    ...
    for (int i = 0; i < n-1; i++)
    {
        ...
        runIndividualCmd(tokens, st_in, fd[1], i); // executing the command
        if(st_in != 0)
        close(st_in);
    ...
    runIndividualCmd(tokens, st_in, st_out, n-1);
...
void Pipe::runIndividualCmd(vector<vector<string>> &cmdTokens, int prev_out, int next_in, int idx)
{
    ...
    else                               // parent process
    {
        if(idx == 0)
        {
            pgrpID = childpid;          // Making pid of first child as process group ID
